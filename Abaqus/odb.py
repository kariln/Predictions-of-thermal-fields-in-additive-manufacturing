...
#Implement for after deposition as well? CHeck if historical variables are correct. Maybe all frames should be included? #"full_part"
    def get_temperature(self,base_depth, part_name,point1,point2,deposition_pattern,road_width,velocity,basedepth,globalseed,model_name,layer_thickness):
        ...
        self.write('layer_thickness = ' + str(layer_thickness) + '\n')
        self.write('point1 = ' + str(point1) + '\n')
        self.write('point2 = ' + str(point2) + '\n')
        self.write('new_active_nodes = -1\n')
        self.write("dispFile = open('disp.txt','w')\n")
        self.write("dispFile.write('i,t,T,x,y,z,t_i,T_1,T_2,T_3,T_4,T_5,pattern,road_width,v,basedepth,layer_thickness,globalseed,surface,nr_surf_nodes,layerNum\\n')\n")
        ...
        self.get_layerNum()
        self.get_surface()
        self.write("\t\t\t\t\tdispFile.write(str(i) + ',' + str(t) + ',' + str(T) + ',' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(t_i) + ',' + str(hist_temp[0]) + ',' + str(hist_temp[1]) + ',' + str(hist_temp[2]) + ',' + str(hist_temp[3]) + ',' + str(hist_temp[4]) + '," + deposition_pattern + ","+ str(road_width) + ","+ str(velocity) + ","+ str(basedepth)+ ","+ str(layer_thickness)+ ","+str(globalseed) + ",' + str(surface)+',' + str(nr_surf_nodes)+',' + str(layerNum)"+ "+'\\n')\n")
        self.write("dispFile.close()\n")
        
    def get_layerNum(self):
        self.write("\t\t\t\t\theight = base_depth\n")
        self.write("\t\t\t\t\twhile z > height:\n")
        self.write("\t\t\t\t\t\theight += layer_thickness\n")
        self.write("\t\t\t\t\tlayerNum = (height-base_depth)/layer_thickness\n")
        self.write("\t\t\t\t\tif layerNum > current_layer:\n")
        self.write("\t\t\t\t\t\tcurrent_layer = layerNum\n")
    
    def get_surface(self):
        self.write("\t\t\t\t\tnr_surf_nodes = 0 \n")
        self.write("\t\t\t\t\tfor elem in surf_nodes:\n")
        self.write("\t\t\t\t\t\tif elem.label == i:\n")
        self.write("\t\t\t\t\t\t\tsurface = 1\n")
        self.write("\t\t\t\t\t\telse:\n")
        self.write("\t\t\t\t\t\t\tsurface = 0\n")
        self.write("\t\t\t\t\t\tif abs(elem.coordinates[0] - x) < 3*road_width and abs(elem.coordinates[1] - y) < 3*road_width and abs(elem.coordinates[2] - z) < 3*road_width:\n")
