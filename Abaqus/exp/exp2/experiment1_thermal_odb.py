...
dispFile.write('i,t,T,x,y,z,Q_x,Q_y,Q_z,d_Q_x,d_Q_y,d_Q_z,t_i,euclidean_d_Q,Q,d_top,d_bottom,d_x1,d_x2,d_y1,d_y2,category,T_1,T_2,T_3,T_4,T_5, pattern,dT_12,dT_23,dT_34,dT_45,road_width,v\n')
stepName = odb.steps.keys()[0]
		...
		if active_nodes != []:
			for i in range(0,len(temperature.values)):
				...
				if temp.nodeLabel in active_nodes:
					...
					dispFile.write(str(i) + ',' + str(t) + ',' + str(T) + ',' + str(x) + ',' + str(y) + ',' + str(z) + ',,,,,,,' + str(t_i) + ',,,'  + str(d_top) + ',' + str(d_bottom)+ ',' + str(d_x1) + ',' + str(d_x2) + ',' + str(d_y1) + ',' + str(d_y2) + ',' + category + ',' + str(hist_temp[0]) + ',' + str(hist_temp[1]) + ',' + str(hist_temp[2]) + ',' + str(hist_temp[3]) + ',' + str(hist_temp[4]) + ',raster,,,,,0.01,0.015\n')
dispFile.close()
