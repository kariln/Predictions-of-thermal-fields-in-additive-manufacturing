		...
		if active_nodes != []:
			for i in range(0,len(temperature.values)):
				...
				if temp.nodeLabel in active_nodes:
					...
					dispFile.write(str(i) + ',' + str(t) + ',' + str(T) + ',' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(t_i) + ',' + str(hist_temp[0]) + ',' + str(hist_temp[1]) + ',' + str(hist_temp[2]) + ',' + str(hist_temp[3]) + ',' + str(hist_temp[4]) + ',zigzag,0.005,0.015,0.02,0.0015,0.005,' +',' + str(layerNum)+'\n')
dispFile.close()
