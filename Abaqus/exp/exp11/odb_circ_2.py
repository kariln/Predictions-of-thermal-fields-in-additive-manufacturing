# -*- coding: utf-8 -*-
"""
Created on Fri Apr 16 10:53:59 2021
...
road_width = 0.002
globalseed = 0.00023
layer_thickness = 0.00138
v = 0.0015
part = mdb.models['thermal'].parts['Part-1']
        ...
        if active_nodes != []:
            for i in range(0,len(temperature.values)):
                ...
                if temp.nodeLabel in active_nodes:
                    ...
                    y = pos.data[1]
                    z = pos.data[2]
                    dispFile.write(str(i) + ',' + str(t) + ',' + str(T) + ',' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(t_i) + ',' + str(hist_temp[0]) + ',' + str(hist_temp[1]) + ',' + str(hist_temp[2]) + ',' + str(hist_temp[3]) + ',' + str(hist_temp[4]) + ',' + str(road_width) +',' + str(globalseed) + ',' + str(v) + ',' + str(base_depth) + ',' + str(layer_thickness) + ',' + str(layerNum) +','  +str(surface) + ',' + str(nr_surf_nodes) +',circular\n')
dispFile.close()