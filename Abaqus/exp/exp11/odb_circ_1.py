# -*- coding: utf-8 -*-
"""
Created on Thu Mar 18 12:17:51 2021
...
dispFile.write('i,t,T,x,y,z,Q_x,Q_y,Q_z,d_Q_x,d_Q_y,d_Q_z,t_i,euclidean_d_Q,Q,T_1,T_2,T_3,T_4,T_5, pattern,dT_1,dT_2,dT_3,dT_4,road_width\n')
stepName = odb.steps.keys()[0]
		...
		if active_nodes != []:
			for i in range(0,len(temperature.values)):
				...
				if temp.nodeLabel in active_nodes:
					...
					y = pos.data[1]
					z = pos.data[2]
					dispFile.write(str(i) + ',' + str(t) + ',' + str(T) + ',' + str(x) + ',' + str(y) + ',' + str(z) + ',,,,,,,' + str(t_i) + ',,,' + str(hist_temp[0]) + ',' + str(hist_temp[1]) + ',' + str(hist_temp[2]) + ',' + str(hist_temp[3]) + ',' + str(hist_temp[4]) + ',raster,,,,,,0.005\n')
dispFile.close()