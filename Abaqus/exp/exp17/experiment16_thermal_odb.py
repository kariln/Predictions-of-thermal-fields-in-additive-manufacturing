...
odb = openOdb('experiment16_thermal.odb')
				...
				if temp.nodeLabel in active_nodes:
					...
					for elem in surf_nodes:
						...
						if abs(elem.coordinates[0] - x) < 3*road_width and abs(elem.coordinates[1] - y) < 3*road_width and abs(elem.coordinates[2] - z) < 3*road_width:
							...
							pass
					dispFile.write(str(i) + ',' + str(t) + ',' + str(T) + ',' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(t_i) + ',' + str(hist_temp[0]) + ',' + str(hist_temp[1]) + ',' + str(hist_temp[2]) + ',' + str(hist_temp[3]) + ',' + str(hist_temp[4]) + ',raster,0.01,0.015,0.02,0.0023,0.005,' + str(surface)+',' + str(nr_surf_nodes)+',' + str(layerNum)+'\n')
dispFile.close()
