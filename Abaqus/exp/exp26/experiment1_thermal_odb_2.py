...
dispFile.write('i,t,T,x,y,z,t_i,T_1,T_2,T_3,T_4,T_5,road_width,v,basedepth,layer_thickness,layerNum\n')
#GET TEMPERATURE
base_depth = 0.02
road_width = 0.005
		...
		if active_nodes != []:
			for i in range(0,len(temperature.values)):
				...
				if temp.nodeLabel in active_nodes:
					...
					dispFile.write(str(i) + ',' + str(t) + ',' + str(T) + ',' + str(x) + ',' + str(y) + ',' + str(z) + ',' + str(t_i) + ',' + str(hist_temp[0]) + ',' + str(hist_temp[1]) + ',' + str(hist_temp[2]) + ',' + str(hist_temp[3]) + ',' + str(hist_temp[4]) + ',0.01,0.015,0.02,0.0023,' + str(layerNum)+'\n')
dispFile.close()
