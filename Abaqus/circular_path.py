...
def circle_path(radius, seeds, area,power):
    ...
    for i in range(1,seeds+1):
        ...
        path_A.append(area)
        path_Q.append(power)
        x = np.cos(2*np.pi/30*i)*radius
        y = np.sin(2*np.pi/30*i)*radius
...
def generate_heat_path(power, path_X,path_Y,path_t, path_Q):
    heat_path = open("heat_path.txt","a")
    for i in range(0,len(path_X)):
        heat_path.write(coord_string(path_t[i], path_X[i], path_Y[i], 7.3*10**(-3), path_Q[i]))
        
def generate_material_path(area, path_X,path_Y,path_t, path_A):
    material_path = open("material_path.txt","a")
    for i in range(0,len(path_X)):
        material_path.write(coord_string(path_t[i], path_X[i], path_Y[i], 7.3*10**(-3), path_A[i]))
